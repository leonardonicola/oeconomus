  ...
  <q-page class="row justify-center col-12">
    <q-dialog @hide="resetEditValues" v-model="openExpenseInfos">
      <q-card flat class="bg-grey-1" style="width: 400px; max-width: 80vw">
        <q-card-section class="bg-red-13 text-white">
          ...
          <q-tab-panel name="infos">
            <q-card-section>
              ...
              <div class="text-body1">
                {{ formatted(entertainment.historic[expenseId].value) }}
              </div>
            </q-card-section>
            ...
            <q-card-section>
              ...
              <div class="text-body1">
                {{ entertainment.historic[expenseId].date }}
              </div>
            </q-card-section>
            ...
            <q-card-section>
              ...
              <div class="text-body1 text-capitalize">
                {{ entertainment.historic[expenseId].payment }}
              </div>
            </q-card-section>
          </q-tab-panel>
    ...
    <div class="col-10">
      ...
      <div class="text-h6 q-mt-xl text-italic text-weight-light" v-else>
        Oops, no Entertainment expenses to list!
      </div>
    </div>
  </q-page>
</template>
...
const { entertainment } = storeToRefs(expenseStore)
...
function removeExpense() {
  const paymentType =
    expenseStore['entertainment'].historic[expenseId.value].payment
  const expenseValue =
    expenseStore['entertainment'].historic[expenseId.value].value
